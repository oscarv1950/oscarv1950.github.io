...
function isNoiseTranslation(text) {
  ...
  const t = text.trim();
  if (!t) return true;
  // Empieza y el contenido entero es una nota entre [ ] o ( )
  if (/^[\[(][^\])]*[\])]:?$/.test(t)) return true;
  // Notas de forma gramatical: "past & past p X", "pres have", "[crit] ..."
  if (/^(past|pres|pp|ger)\b/i.test(t)) return true;
  if (/^\[(crit|colloq|fml|frml|liter|ant|br|us)\]/i.test(t)) return true;
  // Demasiado corta y sin letras (solo puntuación/símbolos)
...
function lookupWord(wordLower) {
  ...
  oxSensesAll.forEach(os => {
    const oxNoisy = isNoiseTranslation(os.traduccion);
    ...
    // Si la traduccion de Oxford es ruido, solo conservamos el sense cuando
    // NOAD aporto una definicion real para esa misma categoria; si no, se omite.
    ...
    senses.push({
      ...
      traduccion:    oxNoisy ? '' : os.traduccion,
...
function lookupPhrasalVerbData(rootLower, phraseKey) {
  ...
  sources.forEach(entry => {
    ...
    if (!pv) return;
    const noisy = isNoiseTranslation(pv.traduccion);
    if (pv.traduccion && !noisy) {
      senses.push({
        categoria: 'phrasal_verb',
        traduccion: pv.traduccion,
        definicion_en: '',
