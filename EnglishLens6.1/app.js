 ...
 *  9. DICTIONARY ENGINE — Oxford ES-EN + tokenizador local
...
/** Formas irregulares inglesas → forma base (para lookup) */
...
/** Estado del motor */
const dictEngine = {
  oxford:  null,   // Map<word_lower, entry>
  pvIndex: null,   // { root: [{parts, hasGap}] }
  inflIdx: null,   // Map<inflected_form, base_word>
  loading: null,
  ready:   false,
};

/** Carga el Oxford ES-EN + índice de phrasal verbs. Segura para llamar múltiples veces. */
function loadDictionaries() {
  if (dictEngine.ready)   return Promise.resolve();
  if (dictEngine.loading) return dictEngine.loading;

  dictEngine.loading = (async () => {
    const [oxRes, pvRes] = await Promise.all([
      fetch(DICT_URLS.oxford),
      fetch(DICT_URLS.pv),
    ]);
    if (!oxRes.ok || !pvRes.ok)
      throw new Error('No se pudo cargar el diccionario. Revisa tu conexión.');

    const [oxArr, pvIndex] = await Promise.all([oxRes.json(), pvRes.json()]);

    const oxMap = new Map();
    const inflIdx = new Map();

    // Cargar formas irregulares estáticas primero (pueden ser sobreescritas
    // por entradas reales del diccionario si existen)
    Object.entries(IRREGULAR_FORMS).forEach(([form, base]) => {
      if (!inflIdx.has(form)) inflIdx.set(form, base);
    });

    oxArr.forEach(e => {
      const key = e.word.toLowerCase();
      oxMap.set(key, e);
      // Índice inverso de inflexiones regulares declaradas en el diccionario
      (e.inflections || []).forEach(infl => {
        const k = infl.toLowerCase();
        if (!inflIdx.has(k)) inflIdx.set(k, key);
      });
    });

    dictEngine.oxford  = oxMap;
    dictEngine.pvIndex = pvIndex;
    dictEngine.inflIdx = inflIdx;
...
function dictionariesReady() { return dictEngine.ready; }


/* ── Limpieza de traducciones del Oxford ES-EN ── */

/**
 * Limpia el texto de traducción:
 * - Elimina marcadores de contexto ‹...› (ej. ‹family/friend›)
 * - Elimina prefijos de registro: [frml], [liter], [colloq], etc.
 * - Elimina prefijos de concordancia: (+ sing vb), (+ pl vb), etc.
 * - Devuelve null si queda sin contenido real en español.
 */
function cleanTranslation(text) {
  ...
  // Ruido puro: toda la cadena es una nota entre [ ] o ( )
  if (/^[\[(][^\])]*[\])]:?\s*$/.test(t)) return null;
  // Notas de forma verbal sin traducción: "past & past p X", "pres have"
  if (/^(past|pres|pp|ger)\b/i.test(t)) return null;

  // Quitar prefijos de registro: [frml], [liter], [colloq], [u], [c], [fml], etc.
  t = t.replace(/^\[(?:crit|colloq|fml|frml|liter|ant|br|us|u|c)\]\s*/gi, '').trim();
  // Múltiples prefijos encadenados: "[c] [frml] texto" → "texto"
  t = t.replace(/^(\[[^\]]+\]\s*)+/g, '').trim();

  // Quitar prefijo de concordancia gramatical: "(+ sing vb)", "(+ pl vb)", "(+ adv compl)"
  t = t.replace(/^\(\+[^)]+\)\s*:?\s*/i, '').trim();
  // Segunda pasada tras quitar registro
  if (/^(past|pres|pp|ger)\b/i.test(t)) return null;

  // Quitar marcadores de contexto ‹objeto› (ej. ‹family/friend› abandonar → abandonar)
  t = t.replace(/‹[^›]*›\s*/g, '').replace(/^\s*[;,]\s*/, '').trim();

  // Cortar en el primer bullet • (ejemplos que se colaron en el campo de traducción)
  const bulletIdx = t.indexOf('•');
  if (bulletIdx !== -1) t = t.slice(0, bulletIdx).trim();

  // Cortar en el primer | (separador de ejemplo bilingüe que se coló)
  const pipeIdx = t.indexOf(' | ');
  if (pipeIdx !== -1) t = t.slice(0, pipeIdx).trim();

  if (!t || !/[a-záéíóúñüà]/i.test(t)) return null;

  // Limpiar artefactos de puntuación sobrantes al inicio/fin
...
/* ── Búsqueda en el Oxford ES-EN ── */

/** Busca una palabra y devuelve sus acepciones limpias, o null si no está. */
function lookupWord(wordLower) {
  const entry = dictEngine.oxford.get(wordLower);
  if (!entry) return null;

  const senses = [];
  (entry.senses || []).forEach(os => {
    const traduccion = cleanTranslation(os.traduccion);
    if (!traduccion) return;   // omitir acepciones sin traducción real

    const ejemplos = (os.ejemplos || [])
      .map(ex => ex.en && ex.es ? `${ex.en} — ${ex.es}` : '')
      .filter(Boolean)
      .slice(0, 3);
  ...
  return { senses, phonetic: entry.phonetic || '' };
}

/** Forma base de una palabra (para lookup de inflexiones). */
function baseFormOf(wordLower) {
  // 1. Tabla estática de irregulares
  const irregular = IRREGULAR_FORMS[wordLower];
  if (irregular) return irregular;
  // 2. Índice del diccionario (inflexiones regulares declaradas en Oxford)
  const fromIndex = dictEngine.inflIdx.get(wordLower);
  if (fromIndex && fromIndex !== wordLower) return fromIndex;
  // 3. Heurísticas de desinflexión regular
  if (wordLower.endsWith('ies'))   return wordLower.slice(0, -3) + 'y';
  if (wordLower.endsWith('ied'))   return wordLower.slice(0, -3) + 'y';
  if (wordLower.endsWith('ing')) {
    const stem = wordLower.slice(0, -3);
    if (dictEngine.oxford.has(stem))    return stem;
    if (dictEngine.oxford.has(stem+'e')) return stem + 'e';
  }
  ...
  if (wordLower.endsWith('es') && dictEngine.oxford.has(wordLower.slice(0, -2)))
    return wordLower.slice(0, -2);
  if (wordLower.endsWith('s')  && dictEngine.oxford.has(wordLower.slice(0, -1)))
    return wordLower.slice(0, -1);
  return null;
}

/** Busca por la forma base si la forma directa no tiene entrada. */
...
/** Busca los datos de un phrasal verb en la entrada de su verbo raíz. */
function lookupPhrasalVerbData(rootLower, phraseKey) {
  const entry = dictEngine.oxford.get(rootLower);
  if (!entry) return null;
  ...
  const traduccion = cleanTranslation(pv.traduccion);
  if (!traduccion && !(pv.ejemplos || []).length) return null;

  const ejemplos = (pv.ejemplos || [])
    .map(ex => ex.en && ex.es ? `${ex.en} — ${ex.es}` : '')
    .filter(Boolean)
    .slice(0, 2);
...
/** Divide el texto en palabras con sus posiciones originales. */
...
/** Intenta detectar un phrasal verb empezando en rootLower con las palabras siguientes. */
function matchPhrasalVerb(rootLower, followingWordsLower) {
  const candidates = dictEngine.pvIndex[rootLower];
  if (!candidates || !candidates.length) return null;

  for (const cand of candidates) {
    const particles = cand.parts.slice(1);
    if (cand.hasGap) {
...
/** Tokeniza el texto completo usando el Oxford ES-EN. */
function tokenizeText(text) {
  ...
  while (i < rawWords.length) {
    ...
    // 1. Intentar phrasal verb (usando forma base del verbo raíz)
    const rootLower  = IRREGULAR_FORMS[wordLower] || baseFormOf(wordLower) || wordLower;
    const following  = lowerWords.slice(i + 1, i + 5);
    ...
    // 2. Palabra individual: buscar directa o por forma base
...
/** Construye el objeto token con campos planos (compat.) + senses[] completo. */
function buildToken(texto_original, tipo, data) {
  const senses    = data ? data.senses : [];
  const categoria = senses.length ? (senses.find(s => s.categoria) || senses[0]).categoria : '';

  // Para los campos planos, preferir el primer sense de la misma categoría
  // que tenga traducción; si no, el primero con traducción de cualquier categoría.
  const pool    = senses.filter(s => s.categoria === categoria);
  const first   = (pool.find(s => s.traduccion) || senses.find(s => s.traduccion) || senses[0] || {});
