...
function renderTokens(tokens) {
  ...
  tokens.forEach((token, i) => {
    ...
    // Espaciado natural entre tokens (sin doble espacio antes de puntuación)
    if (i < tokens.length - 1) {
      const nextText = tokens[i + 1]?.texto_original || '';
      if (!/^[.,;:!?'"\)\]\}]/.test(nextText)) {
...
const fc = {
  ...
  flipped: false,
  correct: 0,    // sabidas en esta sesión
  wrong:   [],   // entradas marcadas como "no sabida"
};
...
function nextFcCard(knew) {
  if (knew) { fc.correct++; }
  else       { fc.wrong.push(fc.deck[fc.index]); }
  fc.index++;
...
function showFcResults() {
  ...
  dom.fcResultsScore.innerHTML   =
    `Sabidas: <strong>${fc.correct} / ${total}</strong> (${pct}%)`;
